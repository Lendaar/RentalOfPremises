...
namespace RentalOfPremises.Services.Implementations
{
    public class TenantService : ITenantService, IServiceAnchor
    {
        ...
        async Task ITenantService.DeleteAsync(Guid id, CancellationToken cancellationToken)
        {
            ...
            var contracts = await contractReadRepository.GetByIdTenantsAsync(id, cancellationToken);
            foreach (var contract in contracts)
            {
                contractWriteRepository.Delete(contract);
                var room = await roomReadRepository.GetByIdAsync(contract.RoomId, cancellationToken);
                if (room != null)
                {
                    room.Occupied = false;
